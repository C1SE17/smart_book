...
// Controller xử lý toàn bộ thao tác với giỏ hàng
class CartController {
    ...
    static addToCart(req, res) {
        ...
        console.log('Parsed data:', { userId, book_id, quantity });

        CartModel.addToCart(userId, book_id, quantity, (err, result) => { // Gọi hàm model với callback
            if (err) { // Xử lý lỗi nếu có
                console.error('Lỗi từ CartModel:', err); // In lỗi ra console để debug
    ...
    static getCartDetails(req, res) {
        ...
        CartModel.getCartDetails(userId, (err, cartDetails) => { // Gọi model với callback
            if (err) { // Xử lý lỗi
                console.error('Lỗi lấy chi tiết giỏ hàng:', err.message); // Log lỗi chi tiết
                return res.status(500).json({ error: 'Lỗi khi lấy chi tiết giỏ hàng: ' + err.message });
            }
            console.log('Lấy chi tiết giỏ hàng thành công:', cartDetails);
