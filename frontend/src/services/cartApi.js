...
class CartApiService extends BaseApiService {
  ...
  async updateCartItem(cartItemId, quantity) {
    try {
      ...
      const result = await this.apiCall(`/cart/items/${cartItemId}`, {
        method: 'PUT',
  ...
  async clearCart(userId) {
    try {
      console.log(`🛒 [CartAPI] Đang xóa toàn bộ giỏ hàng của user ID: ${userId}`);
      const result = await this.apiCall(`/cart/${userId}`, {
        method: 'DELETE'
      });
      ...
      console.error(`💥 [CartAPI] Lỗi khi xóa giỏ hàng của user ID ${userId}:`, error);
      throw error;
    }
  }
}
