...
const ForgotPassword = ({ onBackToLogin, onBackToHome }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Tìm mã code ở nhiều vị trí có thể
      const debugCode = response?.debugCode || response?.data?.debugCode || response?.code;
      
      console.log('response?.debugCode:', response?.debugCode);
      console.log('response?.data?.debugCode:', response?.data?.debugCode);
      console.log('Found debugCode:', debugCode);
      
      // Hiển thị mã code xác thực trong console - LUÔN HIỂN THỊ
      if (debugCode) {
        // Sử dụng nhiều phương thức để đảm bảo hiển thị
        console.log('');
        console.log('VERIFICATION CODE');
        console.log('Mã code:', debugCode);
        console.log('Email:', formData.email);
        console.log('Thời gian hết hạn:', response?.expiresIn || response?.data?.expiresIn, 'giây');
        console.log('');
        
        // Hiển thị đơn giản nhiều lần
        console.log('VERIFICATION CODE:', debugCode);
        console.log('VERIFICATION CODE:', debugCode);
        console.log('VERIFICATION CODE:', debugCode);
        
        // Sử dụng console.info và console.warn
        console.info('VERIFICATION CODE:', debugCode);
        console.warn('VERIFICATION CODE:', debugCode);
        console.error('VERIFICATION CODE:', debugCode);
      } else {
      ...
      <div>
        ...
        <div className="card shadow-lg border-0" style={{ width: '100%', minHeight: '600px', borderRadius: '10px' }}>
          <div className="card-body p-5 d-flex flex-column justify-content-center align-items-center text-center">
            ...
            {/* Success Message */}
            <h3 className="fw-bold text-dark mb-3">Success!</h3>
