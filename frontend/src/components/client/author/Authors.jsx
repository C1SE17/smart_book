...
const Authors = ({ onNavigateTo, onNavigateToAuthorDetail }) => {
    ...
    const loadAuthors = async () => {
        try {
            setLoading(true);
            setError(null);
            console.log(' Bắt đầu load authors...');
            ...
            if (response && response.success) {
                ...
                console.log(' Load thành công', response.data.length, 'tác giả');
            } else {
                const errorMsg = response?.message || t('authors.messages.loadFailed');
                setError(errorMsg);
                console.error(' Load thất bại:', errorMsg);
            }
        } catch (err) {
            console.error(' Error loading authors:', err);
            setError(t('authors.messages.connectionError', { message: err.message }));
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="container mt-4">
                <div className="d-flex justify-content-center">
                    <div className="spinner-border" role="status">
                        <span className="visually-hidden">{t('authors.messages.loading')}</span>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="row">
                <div className="col-12">
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h2 className="mb-0">{t('authors.title')}</h2>
                    ...
                    {filteredAuthors.length === 0 ? (
                        <div className="text-center py-5">
                            ...
                            <p className="text-muted">{t('authors.messages.empty')}</p>
                        </div>
                    ) : (
                        <div className="row">
