...
const AuthorDetail = ({ onNavigateTo, authorId }) => {
    ...
    const loadAuthorDetail = async () => {
        try {
            ...
            if (response.success) {
                ...
                setError(t('authorDetail.messages.loadFailed'));
            }
        } catch (err) {
            console.error('Error loading author detail:', err);
            setError(t('authorDetail.messages.connectionError'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleBookClick = (bookId) => {
        console.log('Clicking on book ID:', bookId);
    ...
    const handleAddToCart = async (book, e) => {
        ...
        try {
            ...
            if (!user || !token) {
                window.showToast(t('authorDetail.messages.loginRequired'), 'error');
                onNavigateTo('auth');
                return;
            }
            ...
            if (response && (response.message || response.success)) {
                window.showToast(t('authorDetail.messages.addToCartSuccess', { title: book.title }), 'success');
                console.log('Book added to cart successfully:', response);
            } else {
                window.showToast(t('authorDetail.messages.addToCartError'), 'error');
                console.error('Failed to add book to cart:', response);
            }
        } catch (error) {
    ...
    const handleAddToWishlist = async (book, e) => {
        ...
        try {
            ...
            const user = JSON.parse(localStorage.getItem('user'));
            if (!user) {
                window.showToast(t('authorDetail.messages.wishlistLoginRequired'), 'error');
                onNavigateTo('auth');
                return;
            }

            // TODO: Implement wishlist API
            window.showToast(t('authorDetail.messages.addToWishlistSuccess', { title: book.title }), 'success');
            console.log('Book added to wishlist successfully');
        } catch (error) {
            console.error('Error adding book to wishlist:', error);
            window.showToast(t('authorDetail.messages.addToWishlistError'), 'error');
        }
    };
            ...
            <div className="container mt-4">
                ...
                <div className="row mb-3">
                    <div className="col-12">
                        ...
                        >
                        <i className="fas fa-arrow-left me-2"></i>{t('authorDetail.breadcrumbs.back')}
                        </button>
                    </div>
                </div>
                ...
                <div className="d-flex justify-content-center">
                    ...
                    <span className="visually-hidden">{t('authorDetail.messages.loading')}</span>
                    </div>
                </div>
            </div>
        );
    ...
    if (!author) {
        return (
            <div className="container mt-4">
                ...
                {t('authorDetail.messages.notFound')}
                </div>
            </div>
        );
    }
        ...
        <div className="container mt-4">
                                ...
                                <div className="col-md-9">
                                    ...
                                    {author.bio && (
                                        <div className="author-bio-full">
                                            <h5 className="text-muted mb-2">{t('authorDetail.labels.bio')}</h5>
                                    ...
                                    {author.books && author.books.length > 0 && (
                                        <div className="author-stats mt-3">
                                            <span className="badge bg-primary me-2">
                                                ...
                                                {t('authorDetail.labels.bookCount', { count: author.books.length })}
                                            </span>
                                        </div>
                                    )}
                                </div>
            ...
            {author.books && author.books.length > 0 ? (
                <div className="row">
                    <div className="col-12">
                        <h3 className="mb-4">
                            {t('authorDetail.labels.worksByAuthor', { name: author.name })}
                        </h3>
                        <div className="row">
                                ...
                                return (
                                    <div key={book.book_id} className="col-lg-3 col-md-6 mb-4">
                                        <div
                                            ...
                                            title={t('authorDetail.tooltips.viewBook')}
                                            onMouseEnter={(e) => {
                                        ...
                                        >
                                            <div className="position-relative">
                                                {book.cover_image && book.cover_image.trim() !== '' ? (
                                                    <img
                                                        ...
                                                        onError={(e) => {
                                                            console.log('Book image failed to load:', book.cover_image);
                                                        }}
                                                        onLoad={() => {
                                                            console.log('Book image loaded successfully:', book.cover_image);
                                                        }}
                                                    />
                                                ) : (
                                                    <div
                                            ...
                                            <div className="card-body d-flex flex-column p-3">
                                                ...
                                                <p className="text-muted small mb-2" style={{
                                                    ...
                                                    {book.category_name || t('authorDetail.labels.unknownCategory')}
                                                </p>
                                                ...
                                                <div className="mt-auto">
                                                    ...
                                                    >
                                                        {t('authorDetail.buttons.viewDetail')}
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
                ...
                <div className="row">
                    <div className="col-12">
                        <div className="text-center py-5">
                            ...
                            <h4 className="text-muted">{t('authorDetail.messages.noBooksTitle')}</h4>
                            <p className="text-muted">{t('authorDetail.messages.noBooksDescription')}</p>
                        </div>
                    </div>
                </div>
            )}
